...
# 输出到 ToV2raynURL/v2rayNURL.txt，生成 map.log 映射日志
param([switch]$Yes)
...
Write-Host "       ToV2raynURL - v2rayN 分享链接生成工具            " -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host " 输出文件: $([IO.Path]::Combine($ProjectRoot, 'ToV2raynURL', 'v2rayNURL.txt'))" -ForegroundColor Green
Write-Host " 映射日志: $([IO.Path]::Combine($ProjectRoot, 'ToV2raynURL', 'map.log'))" -ForegroundColor Green
...
$outDir = [IO.Path]::Combine($ProjectRoot, "ToV2raynURL")
...
$null = $mapLog.AppendLine("ToV2raynURL 转换映射记录")
